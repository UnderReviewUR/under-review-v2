...
import { preferredTournamentScore, isNflInSeason, isNflRampMode, getDaypartLabel } from "../lib/tennis";

function buildHomeQuestions(activeTournamentMatches, tennisLiveMatches, tennisUpcomingMatches, nflSeasonMode, context) {
  const prompts = [];
  const used = new Set();
  ...
  push({ id:"q3", color:"#0891B2",
    text: context?.currentTournament?.name ? `Best futures angle around ${context.currentTournament.name}?` : "Which tennis future still has value right now?",
...
function buildHomeCards(activeTournamentMatches, liveMatches, context, nflSeasonMode, nflRampMode) {
  ...
  if (!tennis.length && liveMatches[0]) {
    tennis.push({ ...liveMatches[0], homeCategory: String(liveMatches[0]?.raw?.live || "0") === "1" ? "Live Tennis" : "Upcoming Tennis" });
  }
  if (!tennis.length) {
    tennis.push({
      ...
      blurb: context?.currentTournament?.context || "Player database, surface Elo, and futures context are loaded.",
  ...
  const nfl = nflSeasonMode
    ? [
        { id:"nfl-season-1", league:"NFL IN-SEASON", leagueColor:"#D97706", title:"Best weekly NFL prop board", time:"Weekly Market", network:"Weekly Props", blurb:"Usage, role changes, and current market mispricing.", whatMatters:"Ask for the best current weekly NFL edge.", quickHitters:["Best prop this week?","Biggest role shift?","Best TD angle?"], confirmed:true },
...
export default function HomeScreen({
  liveMatches, context,
  homeInput, setHomeInput,
  pastedImage, clearImage, isAsking, processImageFile, fileInputRef,
  onSubmitHome, onFirePrompt, onOpenMatchup,
  goTennis, goNfl,
}) {
  ...
  const questions = useMemo(() => buildHomeQuestions(activeTournamentMatches, tennisLiveMatches, tennisUpcomingMatches, nflSeasonMode, context), [activeTournamentMatches, tennisLiveMatches, tennisUpcomingMatches, nflSeasonMode, context]);
  const cards     = useMemo(() => buildHomeCards(activeTournamentMatches, liveMatches, context, nflSeasonMode, nflRampMode), [activeTournamentMatches, liveMatches, context, nflSeasonMode, nflRampMode]);
  ...
  return (
    <main className="screen">
      ...
      <AskBar inputRef={inputRef} fileInputRef={fileInputRef} value={homeInput} onChange={setHomeInput} onSubmit={onSubmitHome} placeholder="Ask UR TAKE anything..." pastedImage={pastedImage} clearImage={clearImage} isAsking={isAsking} processImageFile={processImageFile} />
      ...
      <section className="section">
        ...
        <div className="matchup-list">
          {cards.map(m => <MatchupCard key={m.id} m={m} onOpen={onOpenMatchup} showCategory />)}
        </div>
      </section>
