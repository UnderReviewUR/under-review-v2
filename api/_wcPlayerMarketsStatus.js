...
import { countRegistryPlayers } from "../shared/wcPlayerRegistry.js";
...
export async function buildWcPlayerMarketsStatus(nowMs = Date.now()) {
  const [players, goldenBootRaw, injuries, matchProps, override] = await Promise.all([
    getDurableJson(WC_PLAYERS_KV_KEY),
  ...
  const registry = countRegistryPlayers(players || {});
  ...
  const byEventId = matchProps?.byEventId && typeof matchProps.byEventId === "object"
    ? matchProps.byEventId
    : {};
  ...
  return {
    ...
    players: {
      lastUpdated: players?.lastUpdated ?? null,
      source: players?.source ?? null,
      ...registry,
    },
    injuries: {
