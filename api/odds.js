...
// Sport keys to try in order — tournament-specific first, then general
...
```
// Find best available US bookmaker line
...
// Prop markets available: player_aces, player_double_faults
// These require individual event calls
const propMarkets = “player_aces,player_double_faults”;
const props = [];

// Only fetch props for first 6 matches to stay within rate limits
...
```
    ...
    for (const book of data.bookmakers || []) {
      for (const market of book.markets || []) {
        for (const outcome of market.outcomes || []) {
          const entry = {
            book: book.key,
            player: outcome.name,
            description: outcome.description, // "Over" or "Under"
            line: outcome.point,              // the actual line number e.g. 3.5
            odds: outcome.price,              // american odds e.g. -115
          };

          if (market.key === "player_aces") {
            // Avoid duplicates — keep first book's line per player/direction
      ...
      // One bookmaker is enough per match for line data
      if (matchProps.aces.length > 0 || matchProps.doubleFaults.length > 0) break;
    }

    if (
      matchProps.aces.length > 0 ||
      matchProps.doubleFaults.length > 0
    ) {
      props.push(matchProps);
    }
  } catch {
    // Silent — not all matches will have prop markets
  }
})
```
