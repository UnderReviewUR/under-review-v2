// ur-take.js — full rewrite, imports all player/course data from _ur-take-data.js
// This keeps ur-take.js lean and maintainable.
// The data file (_ur-take-data.js) contains all PGA_PLAYERS, NBA_PLAYERS, NFL_PLAYERS, etc.
...
function buildGolfSystemPrompt(ctx) {
  ...
  if (currentEvent?.leaderboard?.length > 0) {
    leaderStr = `CURRENT LEADERBOARD (${currentEvent.round}):\n` +
      currentEvent.leaderboard.slice(0, 15).map(p => `${p.position}. ${p.name} (${p.country}) -- ${p.score}${p.thru && p.thru !== "--" ? " | Thru " + p.thru : ""}`).join("\n");
  }
  let oddsStr = "";
  ...
  Form: ${(p.recentForm || []).join(", ")} | Cut: ${p.cutMaking||"?"} | Top10: ${p.top10Rate||"?"} | Win: ${p.winRate||"?"}
  Markets: ${(p.bestMarkets||[]).join(", ")} | Note: ${p.note||""}`;
    }).filter(Boolean).join("\n\n");
  ...
  const eventAnchor = currentEvent?.name
    ? `THIS WEEK'S EVENT: ${currentEvent.name}\nCourse: ${currentEvent.course} -- ${currentEvent.location}\nRound: ${currentEvent.round}\n\n`
    : "CURRENT EVENT: Not loaded -- use player SG profiles for best available lean.\n\n";

  return `You are Under Review -- the sharpest golf betting intelligence tool available.

${eventAnchor}IDENTITY: Sharp golf analyst. Lead with the lean. Recommendation first, data second. NEVER ask for more information. Make a call.
...
3. Adjust for recent form: recent WIN or T2-T5 = +10%. Recent missed cuts = -15%.
4. Convert probability to American odds (prob < 50%: odds = +((100/prob)-100), round to nearest 25).
5. Output this exact line: UR FAIR ODDS: [number] | MARKET: [actual line or "not loaded"] | VALUE: [description]
...
[2-4 sentences reasoning -- SG data, course fit, form]
THE PLAY:
[Player] -- [Market]
...
2. Cite exact prop lines and odds when loaded.
3. NEVER recommend props for FINAL games.
4. NEVER use markdown. Plain text only.
5. No games? Give the best NBA FUTURES angle instead. Name specific player and bet.
...
3. Cite actual prop line numbers.
4. No props for FINAL games.
5. No games today? Give the best MLB futures or upcoming matchup angle.
