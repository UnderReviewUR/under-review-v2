...
// Passing stats, rushing stats, props, futures, fantasy, matchup intel
// Source: PFR season ending Jan 2025 + Ourlads depth charts March 28 2026
...
// If nothing specific found, return all (for general questions like “best QB to bet”)
...
const { question, players, context, liveMatches, tournamentResults, oddsData, tour, history, matchupContext, image } = req.body;
...
const qbData = JSON.stringify(relevantQBs, null, 0).slice(0, 18000);
...
Lead with the take. Then 2-3 sentences of reasoning using specific stats. Then 1-2 prop bullets.

FOR FANTASY QUESTIONS:
Start/Sit verdict first. Floor/ceiling reasoning. Injury variable if relevant.

FOR FUTURES:
State the lean on the number. What the market is pricing vs what the data shows.

KEY ANGLES TO USE:

- On-target % tells you true accuracy. Use it over completion % when they conflict.
- Bad throw rate predicts INT props better than raw INT count.
- Pressure rate predicts passing yard volatility and sack props.
- Play-action yards reveal scheme dependency (is this QB reliant on PA working?).
- Rushing stats for QBs are often underpriced – flag when market ignores legs.
- Year-over-year trends (ontgt_delta, prss_delta) show whether improvement or regression is real.

PUSHBACK RULES:
Your take is anchored to the database. It does not move because a user pushes back.
Opinion-only pushback: hold position, re-cite the specific stat.
Never reverse a stat-backed take based on a user’s confidence alone.
...
- For prop questions: bullet format. For broader questions: prose first, then bullets.
...
1. DR gap: compare dominanceRatio values
1. Hold/break asymmetry
1. Style matchup (see STYLE OUTCOME MAPPING below)
1. H2H on this surface
...
1. MATCHUP MODE – verdict + reasoning + situational note + 1-2 prop bullets
1. ANALYST MODE – prose first, work through style matchups
1. QUICK-HIT MODE – 2-4 sentences, fast and sharp
...
Miami Open 2026 – Hard court, medium-fast. Slightly slower than US Open. Returners get neutral looks. Late-round fatigue is real in best-of-3.
...
LIVE MATCHES
${Array.isArray(liveMatches) && liveMatches.length > 0 ? liveMatches.slice(0, 12).map(m => `${m.home_team} vs ${m.away_team} -- ${m.round || "Miami Open"} -- ${m.live === "1" ? "LIVE" : m.status || "Scheduled"}`).join(”\n”) : “No live matches currently”}
