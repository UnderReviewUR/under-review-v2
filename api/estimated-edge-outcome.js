...
import { recordEstimatedEdgeOutcome } from "./_takeLedger.js";
...
export default async function handler(req, res) {
  ...
  try {
    const out = await recordEstimatedEdgeOutcome(email, takeId, {
      result,
      actualLine: req.body?.actualLine,
      notes: req.body?.notes,
    });
    if (!out.ok) {
      if (out.reason === "not_found") return res.status(404).json({ error: out.reason });
      if (out.reason === "no_estimated_edge") return res.status(400).json({ error: out.reason });
      if (out.reason === "bad_result") return res.status(400).json({ error: out.reason });
      return res.status(400).json({ error: out.reason || "bad_input" });
    }
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.warn("[estimated-edge-outcome]", err?.message || err);
